...
const packages = [
  { id: 2048, name: 'Escapada a Bariloche', description: 'Una escapada para disfrutar de los lagos, la montaña y la gastronomía patagónica.', agency: 'Andes Travel', hotel: 'Hotel Patagonia', outboundFlightId: 4512, returnFlightId: 4513, price: 485000, accent: 'coral' },
  { id: 1986, name: 'Rutas de la Toscana', description: 'Recorrido por pueblos medievales, viñedos y ciudades históricas de Italia.', agency: 'Mundo Abierto', hotel: 'Villa Toscana', outboundFlightId: 7821, returnFlightId: 7822, price: 1240000, accent: 'blue' },
  { id: 2011, name: 'Caribe todo incluido', description: 'Descanso frente al mar con alojamiento, comidas y actividades incluidas.', agency: 'Horizonte Turismo', hotel: 'Caribe Resort', outboundFlightId: 6134, returnFlightId: 6135, price: 890000, accent: 'gold' },
  { id: 2072, name: 'Aventura en la Patagonia', description: 'Naturaleza, glaciares y excursiones para descubrir el sur argentino.', agency: 'Andes Travel', hotel: 'Calafate Lodge', outboundFlightId: 5290, returnFlightId: 5291, price: 620000, accent: 'green' },
  { id: 1954, name: 'Japón esencial', description: 'Una primera visita a Japón combinando la energía de Tokio y la tradición de Kioto.', agency: 'Mundo Abierto', hotel: 'Sakura Central Hotel', outboundFlightId: 9044, returnFlightId: 9045, price: 2180000, accent: 'pink' },
]

const Packages = () => {
  const [search, setSearch] = useState('')
  const [agency, setAgency] = useState('Todas las agencias')
  const agencies = ['Todas las agencias', ...new Set(packages.map((item) => item.agency))]
  const filteredPackages = useMemo(() => packages.filter((item) => {
    const matchesAgency = agency === 'Todas las agencias' || item.agency === agency
    const searchableText = `${item.name} ${item.description} ${item.agency} ${item.hotel}`.toLowerCase()
