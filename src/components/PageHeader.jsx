...
export function PageHeader() {
  ...
  return (
    <header className="topbar">
      <a className="brand" href="/" aria-label="Comprá tu Viaje, inicio">
        <span className="brand-mark">CTV</span>
        <span>Comprá tu Viaje</span>
      </a>
      <nav aria-label="Navegación principal">
        {getLinks().map((link) => (
          <Link key={link.path} label={link.label} path={link.path} />
        ))}
      </nav>
      <button
      ...
      <Button variant="outline-danger" size="sm" onClick={doLogout}>
        Cerrar sesión
      </Button>
    </header>
  );
